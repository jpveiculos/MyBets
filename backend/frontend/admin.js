...
function renderUsers(users){
 ...
 $("users").innerHTML=filtered.map(x=>`<div class="admin-row"><span><b>#${x.id} • ${esc(x.username)}</b><small>CPF: ${esc(x.cpf||"Não informado")} • Cadastro: ${dateTime(x.created_at)} • Créditos para jogar: ${Number(x.play_credits||0).toLocaleString("pt-BR",{maximumFractionDigits:2})} • Saldo para saque: ${money(x.withdrawable_balance)} • Reserva: ${money(x.reserved_balance)} • Status: ${x.is_banned?"BANIDO":"ATIVO"}</small></span><span class="row-actions"><button data-id="${x.id}" class="small-btn history-user">Histórico</button><button data-id="${x.id}" class="small-btn add-credits">+ créditos</button><button data-id="${x.id}" class="small-btn add-bonus-credits">+ bônus de crédito</button>${x.is_banned?'<button data-id="'+x.id+'" class="small-btn unban-user">Desbanir</button>':'<button data-id="'+x.id+'" class="small-btn ban-user">Banir</button>'}<button data-id="${x.id}" class="small-btn delete-user">Excluir</button></span></div>`).join("")||'<p class="muted">Nenhum usuário encontrado.</p>';
}
...
$("signupBonusSave")?.addEventListener("click",async()=>{
 ...
 document.querySelectorAll(".add-credits").forEach(b=>b.onclick=async()=>{const v=prompt("Valor a adicionar em créditos (o bônus percentual será aplicado):");if(v)await action("/api/admin/users/"+b.dataset.id+"/credits","POST",{amount:Number(v)})});
