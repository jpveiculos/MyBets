...
function renderRouletteSettings(settings){
 ...
 try{
  const prizes=JSON.parse(map.roulette_prizes??"[2,3,4,5,2,3,4,5]");
...
function bindActions(){
 ...
 $("rouletteSave")?.addEventListener("click",async()=>{
  ...
  if(!Number.isFinite(min)||min<=0||!Number.isFinite(max)||max<min||prizes.length!==8||prizes.some(v=>!Number.isFinite(v)||v<=0)){
   $("rouletteMessage").textContent="Confira mínimo, máximo e os 8 multiplicadores.";
   return;
  }
