...
async function load(){try{const c=await api("/api/games/"+GAME.id+"/config"),a=await api("/api/account");config=c.game;account=a.account;bet=Math.max(config.minBet,1);$("balance").textContent="R$ "+money(account.available_balance);$("bet").textContent=money(bet);initial();updatePlayability()}catch(e){$("result").textContent=e.message||"Faça login para jogar.";$("spin").disabled=true}}
