...
app.get("/api/payments/mercadopago/status/:id", requireUser, asyncRoute(async (req,res) => {
  const result=await syncMercadoPagoDeposit({
...
app.post("/api/credits/purchase", requireUser, asyncRoute(async (req,res) => {
  const result=await purchasePlayCredits({userId:req.user.id,amount:req.body.amount});
  const account=await getAccount(req.user.id);
...
app.put("/api/admin/settings/:key", requireAdmin, asyncRoute(async (req,res) => {
  const editableSettings=new Set([
    ...
    "pix_instructions","audit_log_retention_days","signup_bonus_amount","deposit_bonus_percent","roulette_min_bet","roulette_max_bet"
  ]);
